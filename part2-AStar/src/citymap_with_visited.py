...
class State:
    ...
    def __lt__(self, other):
        ...
        """
        # Implement me
        cost1 = 1.0 * self.get_time() + self.heuristic()
        cost2 = 1.0 * other.get_time() + other.heuristic()
        return cost1 < cost2
    ...
    def heuristic(self):
        ...
        return dist((self.stop['x'], self.stop['y']),
                    (self.goal['x'], self.goal['y']))/MAX_TRAM_SPEED
...
class CityMap:
    ...
    def search(self, start, goal, time_of_beginning):
        ...
        while not to_visit.empty():
            ...
            if pysakki_nyt['code'] in visited:
                continue
            visited.add(pysakki_nyt['code'])
            if pysakki_nyt == goal:
