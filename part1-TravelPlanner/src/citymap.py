...
class CityMap:
    ...
    def search(self, start, goal):
        ...
        visited = {}
        while not q.empty():
            kasiteltava_tila = q.get()
            pysakki_nyt = kasiteltava_tila.get_stop()
            if pysakki_nyt == goal:
                return kasiteltava_tila
            visited[pysakki_nyt] = True
            for naapuri in self.get_neighbors(pysakki_nyt):
                if naapuri not in visited:
                    q.put(State(naapuri, kasiteltava_tila))
        return None
